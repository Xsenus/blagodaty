...
public sealed record DatabaseBackupOverview(
    ...
    int AdminTelegramRecipientsCount,
    IReadOnlyList<DatabaseBackupFileInfo> Items);

public sealed record DatabaseBackupDeliveryResult(
    string FileName,
    int CandidateRecipients,
    int DeliveredCount);

public sealed record DatabaseBackupCreateResult(
    DatabaseBackupFileInfo Backup,
    string DownloadPath,
    DatabaseBackupDeliveryResult? Delivery);
...
public sealed class DatabaseBackupService
{
    ...
    public async Task<DatabaseBackupOverview> GetOverviewAsync(CancellationToken cancellationToken = default)
    {
        ...
        var recipientsCount = await CountAdminTelegramRecipientsAsync(cancellationToken);
        ...
        return new DatabaseBackupOverview(
            ...
            settings.TelegramDeliveryEnabled,
            recipientsCount,
            items);
    }
    ...
    public async Task<DatabaseBackupCreateResult> CreateManualBackupAsync(
        bool sendToAdmins,
        CancellationToken cancellationToken = default)
    {
        var backup = await CreateBackupAsync("manual", null, cancellationToken);
        DatabaseBackupDeliveryResult? delivery = null;

        if (sendToAdmins)
        {
            delivery = await SendBackupToAdminsCoreAsync(backup, throwWhenNoRecipients: true, cancellationToken);
        }

        return new DatabaseBackupCreateResult(
            backup,
            $"/api/admin/backups/download?relativePath={Uri.EscapeDataString(backup.RelativePath.Replace('\\', '/'))}",
            delivery);
    }

    public async Task<DatabaseBackupDeliveryResult> SendBackupToAdminsAsync(
        string? relativePath,
        CancellationToken cancellationToken = default)
    {
        var backup = string.IsNullOrWhiteSpace(relativePath)
            ? (await ListBackupsAsync(cancellationToken)).FirstOrDefault()
            : BuildFileInfo(ResolveBackupPath(relativePath) ?? string.Empty, ResolveRootDirectory());

        if (backup is null)
        {
            throw new InvalidOperationException("Файл резервной копии не найден.");
        }

        return await SendBackupToAdminsCoreAsync(backup, throwWhenNoRecipients: true, cancellationToken);
    }
    ...
    public async Task<int> EnsureScheduledBackupsAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var scheduledTime in settings.Schedule)
        {
            ...
            var backup = await CreateBackupAsync("auto", slotKey, cancellationToken);
            createdCount++;
            ...
            if (settings.TelegramDeliveryEnabled)
            {
                try
                {
                    var delivery = await SendBackupToAdminsCoreAsync(backup, throwWhenNoRecipients: false, cancellationToken);
                    _logger.LogInformation(
                        "Scheduled backup {FileName} delivered to {DeliveredCount}/{RecipientsCount} admin Telegram recipients.",
                        backup.FileName,
                        delivery.DeliveredCount,
                        delivery.CandidateRecipients);
                }
                catch (Exception exception)
                {
                    _logger.LogWarning(exception, "Failed to deliver scheduled backup {FileName} to Telegram admins.", backup.FileName);
                }
            }
        }
    ...
    public string? ResolveBackupPath(string? relativePath)
    {
        ...
        var fullPath = Path.GetFullPath(Path.Combine(rootDirectory, safeRelativePath));
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException("Имя базы данных не задано в строке подключения PostgreSQL.");
            }
            ...
            startInfo.ArgumentList.Add((csb.Port <= 0 ? 5432 : csb.Port).ToString(CultureInfo.InvariantCulture));
            ...
            catch (Win32Exception exception)
            {
                ...
                throw new InvalidOperationException(
                    $"Не удалось запустить pg_dump по пути '{pgDumpExecutable}'. {exception.Message}",
                    exception);
            }
            ...
            using (process)
            {
                ...
                if (process.ExitCode != 0)
                {
                    ...
                    var detail = string.IsNullOrWhiteSpace(standardError)
                        ? $"pg_dump завершился с кодом {process.ExitCode}."
                        : standardError;
                    throw new InvalidOperationException(detail);
                }

                var fileInfo = BuildFileInfo(outputPath, settings.RootDirectory)
                    ?? throw new InvalidOperationException("Резервная копия создана, но не была найдена в файловой системе.");
    ...
    private async Task<DatabaseBackupDeliveryResult> SendBackupToAdminsCoreAsync(
        DatabaseBackupFileInfo backup,
        bool throwWhenNoRecipients,
        CancellationToken cancellationToken)
    {
        var resolvedPath = ResolveBackupPath(backup.RelativePath);
        if (string.IsNullOrWhiteSpace(resolvedPath))
        {
            throw new InvalidOperationException("Файл резервной копии не найден.");
        }

        var recipients = await GetAdminTelegramChatIdsAsync(cancellationToken);
        if (recipients.Count == 0)
        {
            if (throwWhenNoRecipients)
            {
                throw new InvalidOperationException("Среди администраторов нет привязанных Telegram-аккаунтов.");
            }

            return new DatabaseBackupDeliveryResult(backup.FileName, 0, 0);
        }

        using var scope = _scopeFactory.CreateScope();
        var externalAuthProviderService = scope.ServiceProvider.GetRequiredService<ExternalAuthProviderService>();
        var deliveredCount = 0;
        foreach (var chatId in recipients)
        {
            var sent = await externalAuthProviderService.SendTelegramDocumentAsync(
                chatId,
                resolvedPath,
                backup.FileName,
                $"Резервная копия базы данных Blagodaty: {backup.FileName}",
                cancellationToken);
        ...
        if (deliveredCount == 0 && throwWhenNoRecipients)
        {
            throw new InvalidOperationException("Не удалось отправить резервную копию в Telegram администраторам.");
        }

        return new DatabaseBackupDeliveryResult(backup.FileName, recipients.Count, deliveredCount);
    }
    ...
    private Task CleanupExpiredBackupsAsync(int retentionDays, CancellationToken cancellationToken)
    {
        ...
        var thresholdUtc = _timeProvider.GetUtcNow().UtcDateTime.AddDays(-retentionDays);
        foreach (var filePath in Directory.EnumerateFiles(rootDirectory, "*.dump", SearchOption.AllDirectories))
        {
            cancellationToken.ThrowIfCancellationRequested();
    ...
    private async Task<BackupSettings> LoadSettingsAsync(CancellationToken cancellationToken)
    {
        ...
        var rows = await appSettingsService.GetRowsByKeysAsync(DatabaseBackupSettingKeys.All, cancellationToken);

        var automaticEnabled = ParseBoolean(
            GetSettingOrConfig(rows, DatabaseBackupSettingKeys.Enabled, "DatabaseBackup:Enabled"),
            fallback: false);
        var scheduleRaw = GetSettingOrConfig(rows, DatabaseBackupSettingKeys.ScheduleLocal, "DatabaseBackup:ScheduleLocal") ?? "03:00";
        var retentionDays = ParseInt(
            GetSettingOrConfig(rows, DatabaseBackupSettingKeys.RetentionDays, "DatabaseBackup:RetentionDays"),
            fallback: 14,
            minValue: 1,
            maxValue: 365);
        var telegramDeliveryEnabled = ParseBoolean(
            GetSettingOrConfig(rows, DatabaseBackupSettingKeys.TelegramDeliveryEnabled, "DatabaseBackup:TelegramDeliveryEnabled"),
            fallback: false);
        var pgDumpCommand = NormalizeCommandValue(
            GetSettingOrConfig(rows, DatabaseBackupSettingKeys.PgDumpPath, "DatabaseBackup:PgDumpPath"));
        ...
        return new BackupSettings(
            ...
            retentionDays,
            ResolveRootDirectory(GetSettingOrConfig(rows, DatabaseBackupSettingKeys.Directory, "DatabaseBackup:Directory")),
            pgDumpCommand,
            telegramDeliveryEnabled);
    }

    private async Task<int> CountAdminTelegramRecipientsAsync(CancellationToken cancellationToken)
    {
        var recipients = await GetAdminTelegramChatIdsAsync(cancellationToken);
        return recipients.Count;
    }

    private async Task<List<long>> GetAdminTelegramChatIdsAsync(CancellationToken cancellationToken)
    {
        using var scope = _scopeFactory.CreateScope();
        ...
        return await (
            ...
            join identity in dbContext.UserExternalIdentities.AsNoTracking() on userRole.UserId equals identity.UserId
            where role.Name == AppRoles.Admin
                  && identity.Provider == "telegram"
                  && identity.TelegramChatId != null
            select identity.TelegramChatId!.Value
        )
        .Distinct()
    ...
    private string? GetSettingOrConfig(
        IReadOnlyDictionary<string, AppSetting> rows,
        string key,
        string configPath)
    {
        if (rows.TryGetValue(key, out var row))
        {
            var normalizedRowValue = AppSettingsService.NormalizeValue(row.Value);
            if (!string.IsNullOrWhiteSpace(normalizedRowValue))
            {
                return normalizedRowValue;
            }
        }

        return AppSettingsService.NormalizeValue(_configuration[configPath]);
    }

    private string ResolveRootDirectory()
    {
        return ResolveRootDirectory(_configuration["DatabaseBackup:Directory"]);
    }

    private string ResolveRootDirectory(string? configuredPath)
    {
        ...
        return Path.GetFullPath(Path.Combine(_hostEnvironment.ContentRootPath, "backups", "database"));
    }
    ...
    private static string? TryResolveExecutableCandidate(string candidate)
    {
        ...
        {
            return File.Exists(normalizedCandidate)
